...
export default class Model {
  ...
  constructor(scene, camera, seeHelpers, settings) {
    this.seeHelpers = seeHelpers;
    this.initialize(scene, camera, settings);
  }

  initialize(scene, camera, settings) {
//    let model = undefined;
...
//    this.txtPathStart = './../images/';

    // Copy of this to use in nested funtion below
    let self = this;

    let loaderPath = `${settings.txtPathStart}${settings.model}`;
    // Load room
    ...
    loader.load(
      ...
      function(gltf) {
        self.gltfParse(gltf, scene, settings);
...
//    );
  ...
  gltfParse(gltf, scene, settings) {
    let model = gltf.scene;
    this.animations = gltf.animations;
    model.traverse(obj => {
      if (obj.isMesh) {
        /* Use different texture depending on object */
        this.setTextures(obj, settings);
  ...
  setTextures(obj, settings) {
    ...
    if (obj.name.slice(0,5) === "glass") {
      ...
      if (obj.name.slice(0,6) === "season") {
        obj_txt = new THREE.TextureLoader().load(`${settings.txtPathStart}${txtPathMid}seasoning.png`);
        obj_txt.flipY = false;
        this.transparentSkinMesh(obj, obj_txt);
      } else if (obj.name.slice(0,4) === "frig") {
        obj_txt = new THREE.TextureLoader().load(`${settings.txtPathStart}${txtPathMid}frig.png`);
        obj_txt.flipY = false;
        this.texturedMesh(obj, obj_txt);
      } else if (obj.name.slice(0,4) === "cont") {
        obj_txt = new THREE.TextureLoader().load(`${settings.txtPathStart}${txtPathMid}container.png`);
        obj_txt.flipY = false;
        this.texturedMesh(obj, obj_txt);
      } else if (obj.name === "sign" || obj.name === 'bulbs') {
        obj_txt = new THREE.TextureLoader().load(`${settings.txtPathStart}${txtPathMid}${obj.name}.png`);
        obj_txt.flipY = false;
...
//    obj.collider.setFromObject(obj);
    ...
    let obj_mtl = new THREE.MeshStandardMaterial({
      color: 0xA6A6A6,
      skinning: true
  ...
  transparentMesh(obj, op) {
    let obj_mtl = new THREE.MeshPhongMaterial({
      color: 0xF6FEFF,
      opacity: op,
      transparent: true,
      skinning: true
...
//    obj.collider.setFromObject(obj);
  ...
  spotlightMesh(obj, obj_txt) {
    ...
    let obj_mtl = new THREE.MeshMatcapMaterial({
      map: obj_txt,
      color: 0xffffff,
      skinning: true
    });
